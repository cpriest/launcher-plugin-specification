...
The entry object (result from the plugin) tries to answer most of the options launchers have, but the launcher can decide which parts of the result it supports.
...
This format allows us to piggyback on visual studio code's jsonrpc wire format and use existing libraries for talking such as nodejs's vscode-jsonrpc module.
...
The flow section below describes the logic between the messages.
...
After the connection is established, the launcher should advertise it's api version support with the HelloServer message. The plugin should reply back with the selected api version with a HelloClient message.
...
runtime | string | Which external runtime the plugin needs to run
...
the runtime key should exist if the plugin needs an external runtime such as "python" for any version of platform that exists, "python2" or "python3" for a specific version or even "python2.7" or "python3.6" for a more specific version, the same for "nodejs" (other runtime are not supported for now, and it's best that the plugin provides a shell script if needed to find and launch the needed platform or bundle it inside).

the command key is an array of arguments to execute the plugin process, ${RUNTIME} will be replaced with the correct path for the needed runtime on the specific platform.
