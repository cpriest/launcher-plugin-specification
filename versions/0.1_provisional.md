...
In the future there could be generic wrappers for the most popular languages and frameworks that implement the specification.
...
Either side is allowed to ignore messages (or part of their contents) it does not support, however consideration should be taken regarding how it effects the overall experience.

While the protocol might look overwhelming, please read the Flow section to see how most of it can be shimmed away for a simple variant.
...
3. The launcher executes the plugin process, passing command line arguments for the transport connection information.
...
5. The protocol is mainly an exchange of asynchronous messages that are communicated between the processes in response to previous messages as described later in the specification.
...
Each message is a JSON-RPC 2.0 notification that is initiated by the Launcher or the Plugin. All the communication after the HelloServer and HelloClient messages handshake is asynchronous.
...
image | string | local file URI or [Data URI](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URIs) for image
...
name | type | description
---|:---:|---
name | string | name of the plugins
...
the os key should not exist if the plugin is cross-platform and runs on any OS. Otherwise, it should include which platforms it supports. Since the launcher should list only supported platforms, the valid keys should be "windows", "linux", "macos".

the requirements key, should exist if the plugin needs external dependencies such as "python2", "python3", "nodejs".

the command key is an array of arguments to execute the plugin process, where the special keys should be replaced: ${TRANSPORT} with the selected transport (socket/unix) and ${TRANSPORT_ADDRESS} with the address to try connecting to, such as "localhost:1234" (socket) or "/tmp/pluginsock.1" (unix).
...
## Miscellaneous (Should be removed when the version finalizes)
