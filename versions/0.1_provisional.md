...
This message signals the launcher that there are no more results for the request sent with the id in the params. id may be null or empty if the request had no id.
...
* Should we force launcher executing the plugin in a shell context ?
